...
export class NoteWorkspace {
  static _rxTagNoAnchors = '/\\#[\\w-_]+/i';

  static rxTagNoAnchors(): RegExp {
    return new RegExp(/\#[\w\-\_]+/i); // used to match tags that appear within lines
  }
  static rxTagWithAnchors(): RegExp {
    return new RegExp(/^\#[\w\-\_]+$/i); // used to match entire words
  }
  static rxWikiLink(): RegExp {
    return new RegExp(/\[\[[\w\.\-\_\/\\]+/i); // [[wiki-link-regex
  }
  static rxMarkdownWordPattern(): RegExp {
    return new RegExp(/([\#\.\/\\\w_]+)/); // had to add [".", "/", "\"] to get relative path completion working and ["#"] to get tag completion working
  }
